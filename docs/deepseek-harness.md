...
1. 校验本地 DSH 的版本、npm SHA-512 integrity 和实际 CLI 版本。
2. 将桥接按内容哈希发布到独立、不可变的版本目录；安装包升级后，下次启动会自动采用新插件。
3. 检查目标地址的桥接协议与内容版本；完全一致时复用现有 DSH，不会重启它。
4. 地址无人监听时，用 beautiCode 私有运行时一键启动 `dsh web`，等待桥接就绪后打开页面和托盘。
5. 若端口已有旧桥接或未加载桥接，停止并给出错误；不会中断现有会话。用户关闭该 DSH 后再次启动即可加载新插件。
6. 每 24 小时最多查询一次 npm 最新版本。只提示新版本，不会把未经回归验证的 DSH 自动升级为兼容版本。
