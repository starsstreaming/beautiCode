...
> 产品决定（2026-08-16）：**Codex 启动器与托盘保持不变。** 本文件原先「两边启动器都删」的方案已收敛为：只卸 DSH 运行时/启动器，DSH 改为用户自装插件。
>
> 前提：**不要求向后兼容 DSH 内置运行时**。旧 DSH 启动器、私有 `DSH_HOME`、安装包附带的 DSH 可以直接断。
> 目标：DSH 路径从「替代 DSH 的启动器」退化成「插在用户自己 DSH 上的薄插件」；Codex 路径不动。
